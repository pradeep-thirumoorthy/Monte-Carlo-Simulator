...
export interface SimulationInput {
  ...
  years: number;
  meanReturn: number; // nominal return, e.g., 0.07
  volatility: number; // e.g., 0.12
  inflationRate: number; // e.g., 0.03
  managementFee: number; // e.g., 0.01
  targetGoal: number;
}
...
export function runSimulation(input: SimulationInput, numSimulations = 2000): SimulationOutput {
  const { initialInvestment, annualContribution, years, meanReturn, volatility, inflationRate, managementFee, targetGoal } = input;
  
  const allPaths: number[][] = [];
  const finalValues: number[] = [];
  let successCount = 0;

  // Calculate Real Net Return (Nominal - Inflation - Fees)
  const netReturn = meanReturn - inflationRate - managementFee;
  ...
  for (let i = 0; i < numSimulations; i++) {
    ...
    for (let y = 1; y <= years; y++) {
      const z = randn_bm();
      // dt = 1 year using the net return
      const return_t = Math.exp((netReturn - 0.5 * volatility * volatility) + volatility * z);
      currentValue = (Number(currentValue) + Number(annualContribution)) * return_t;
  ...
  const binWidth = (maxFinal - minFinal) / binCount;
  
  const histogram: HistogramBin[] = Array.from({ length: binCount }).map((_, i) => {
    const minVal = minFinal + i * binWidth;
    const maxVal = minFinal + (i + 1) * binWidth;
    return {
      min: minVal,
      max: maxVal,
      count: 0,
      range: `${formatCompact(minVal)} - ${formatCompact(maxVal)}`
    };
  });
